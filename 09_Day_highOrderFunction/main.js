//forEach:allows you to go through each instances of the array,map:allows you to go through each instances of the array but returns the answer in an array,filter:allows you to to set a particular criteria for the instances in the array and returns a boolean answer,reduce:allows You filter every single value and then you map the remaining values. With reduce you can filter and then map in a single pass.//


const countriez = ['Finland', 'Sweden', 'Denmark', 'Norway', 'IceLand']
console.log(countriez.forEach(country => console.log(country)));
...
console.log(countriez.map(country => country.toUpperCase()));

console.log(countriez.map(country => country.length));
...
const productToPrice = products.map(pro => `${pro.product} : ${pro.price}`)
console.log(productToPrice);

const countryWithLand = countriez.filter(country => country.toLocaleLowerCase().includes('land'))
console.log(countryWithLand);

const countriesWithSixChar =countriez.filter(country => country.length==6)
console.log(countriesWithSixChar);

const moreThanSix = countriez.filter(country => country.length > 6)
console.log(moreThanSix);

//No countries start with E,but some countries includes an E//
const countryWithE = countriez.filter(country => country.toUpperCase().includes('E'))
console.log(countryWithE);

const priceValues = products.map(pro => pro.price).filter(val => typeof(val) == "number")
console.log(priceValues);


const reduceSum = numbers.reduce((acc, curr) => acc + curr, 0)
console.log(reduceSum)




const greaterSeven = names.some(a => a.length > 7)
  console.log(greaterSeven);
//ex21
const conAllLand=countriez.every(a=>a.includes('land'))
console.log(conAllLand);
// ex22
//find:this returns the first item that meets the condition//
//findIndex:this returns the index of the first item that meets the condition//


console.log(countriez.find(a => a.length === 6));


console.log(countriez.findIndex(a => a.length === 6));


console.log(countriez.findIndex(a => a === 'Norway'));

//ex26
console.log(countriez.findIndex(a=>a==='Russia'));
...
const namess = ['Mikail', 'Hassan', 'Laja', 'James']
          

const statements = []
for (let i = 0; i < names.length; i++) {
  statements.push(`${namess[i]} is a student`)
}
...
console.log(countriez.sort((a, b) => b.population - a.population).slice(0, 10).map(country => ({ name: country.name, population: country.population })))
